  ...
  _createClass(Formatter, [{
    ...
    value: function getFormattedQueryFromTokens() {
      ...
      this.tokens.forEach(function (token, index) {
        ...
        } else if (token.type === _tokenTypes["default"].EXTRA_INDENT) {
          formattedQuery = _this.formatExtraIndentWord(token, formattedQuery);
          _this.previousReservedWord = token;
        } else if (token.type === _tokenTypes["default"].EXTRA_INDENT_NEWLINE) {
          formattedQuery = _this.formatExtraIndentNewlineWord(token, formattedQuery);
          _this.previousReservedWord = token;
    ...
    key: "formatExtraIndentWord",
    value: function formatExtraIndentWord(token, query) {
      this.indentation.increaseTopLevel();
      query = this.addNewline(query);
      query += this.equalizeWhitespace(this.formatReservedWord(token.value)) + ' ';
      return query;
    ...
    key: "formatExtraIndentNewlineWord",
    value: function formatExtraIndentNewlineWord(token, query) {
      query = this.addNewline(query);
      this.indentation.increaseTopLevel();
      query += this.equalizeWhitespace(this.formatReservedWord(token.value));
      return this.addNewline(query);
