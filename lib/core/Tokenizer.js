...
function () {
  ...
  function Tokenizer(cfg) {
    ...
    this.RESERVED_EXTRA_INDENT_REGEX = cfg.extraIndentWords.length > 0 ? this.createReservedWordRegex(cfg.extraIndentWords) : undefined;
    this.RESERVED_EXTRA_INDENT_NEWLINE_REGEX = cfg.extraIndentNewlineWords.length > 0 ? this.createReservedWordRegex(cfg.extraIndentNewlineWords) : undefined;
  ...
  _createClass(Tokenizer, [{
    ...
    value: function getReservedWordToken(input, previousToken) {
      ...
      return this.getTopLevelReservedToken(input) || this.getExtraIndentToken(input) || this.getExtraIndentNewlineToken(input) || this.getNewlineReservedToken(input) || this.getTopLevelReservedTokenNoIndent(input) || this.getPlainReservedToken(input);
    }
  }, {
    ...
    key: "getExtraIndentToken",
    value: function getExtraIndentToken(input) {
      if (!this.RESERVED_EXTRA_INDENT_REGEX) return undefined;
      return this.getTokenOnFirstMatch({
        input: input,
        type: _tokenTypes["default"].EXTRA_INDENT,
        regex: this.RESERVED_EXTRA_INDENT_REGEX
      });
    }
  }, {
    key: "getExtraIndentNewlineToken",
    value: function getExtraIndentNewlineToken(input) {
      if (!this.RESERVED_EXTRA_INDENT_NEWLINE_REGEX) return undefined;
      return this.getTokenOnFirstMatch({
        input: input,
        type: _tokenTypes["default"].EXTRA_INDENT_NEWLINE,
